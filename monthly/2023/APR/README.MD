...
is hard for non-programmers to maintain, or *Server Page*, which does the job of formatting response.
...
- For view there are two patterns to choose: *Transform View* (XSLT), which translates the result of Domain Model to the
appropriate response form, or *Template View* (JSP), which is a static template with placeholders for dynamic content.
- For either of these two one-step view may be applied, especially where there is one view per screen, or *Two-steps View*,
