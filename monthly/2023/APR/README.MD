...
- The domain model should be created and should iteratively change with a brainstorming sessions between domain experts and developers.
...
- Discovering the domain model should be more than finding nouns, the interactions between objects and applied business rules are equally important.
...
But it provides unnecessary level of details and is sometimes hard to understand: not only developers should understand the model.
...
- Frameworks can get into way, bloating domain objects with infrastructure code. Using only part of framework's functions may be a good choice.
...
Use `Person president = usa.president(1790) // George Washington` instead of `Collection<Person> presidents = usa.presidents()`.
...
- Data source architecture should depend on domain logic architecture. For Transaction Scripts Row Data Gateway or Table Data Gateway
is a good choice, we don't struggle with concurrency problems too much. For Table Module the only choice is Table Data Gateway.
For Domain Model, if the model is relatively small, good choice may be Active Record pattern. For more complicated model - separate
mapping layer with O/R mapping tool.
- Presentation design is independent on previous choices. Good choice is to go with MVC. When the application is document-oriented,
...
- Web services should be used for communications between applications. They should not be used to distribute computations.
...
- Transaction Scripts pattern assumes single procedure for single business operation. It's a good pattern for simple business logic.
...
- Domain Model pattern fits well to the complex business logic. Allows better code structuring. Should use fine-grained interfaces.
It doesn't work well with relational databases, requires using Active Records pattern or O/R mapping layer. It requires practice to implement it well. 
It contains of interconnected objects, containing both data and behaviors. It shows the true power when combined with business rules strategies. 
...
- Table Modules operate on record sets. The pattern offers the benefits of Object-Oriented approach for interacting with objects.
The price is that it doesn't give the full benefits of objects (polymorphism doesn't work well).
Data access objects may be instantiated (better isolation, works better with concurrency), or accessible with static methods.
The client first interacts with Table Data Gateway, and then hands the flow over to Table Modules (for validation, transformation etc.). 
Inserting data requires providing dedicated methods. Accessing particular records requires providing ID to record sets.
- Service Layer defines boundaries of applications. It's a facade between clients and domain logic. It's usually coarse-grained.
May be exposed for remote operations, but it's painful and requires dealing with Data Transfer Objects. 
Method signatures may use domain objects. Using it is optional when the flow doesn't interact with multiple transactional resources
or there is only one kind of client.
Services may be domain facades, simply passing the flow to domain objects, or operational, describing some interactions between domain objects.
...
For software developers it may be a series of repeatable simple tasks, training keystroke, or some discipline (like TDD).
...
one person to write tests and the second to make them pass.
  - *Randori* is a simulated combat training. These can be some real, short, prepared problems, to solve in a group or alone.
...
- It's our own responsibility to practice. We should spend our time and effort for it, it's not employee's responsibility.
...
But the certain level of precision is not achievable: When demonstrating features to stakeholders, they often gain a different vision,
change their minds, so the requirements change. Estimates are not precise by definition. There is a risk of late ambiguity, 
...
- Testing GUI is hard - GUI is constantly changing. It should be approached as testing GUI as API (testing controls using
...
- Acceptance tests should be run daily as part of Continuous Integration. When the build fails, it should be an emergency
...
- API for [Tags search](https://github.com/marcinciapa/actracker-api/pull/39) was introduced. It is used to [search for tags](https://github.com/marcinciapa/actracker-ui/pull/12) when adding/editing activity.
- Get all Tags API calls were [replaced](https://github.com/marcinciapa/actracker-ui/pull/13) in UI with search Tags and resolve Tags API.
...
It was used by UI to implement [switching to activity with selected tag](https://github.com/marcinciapa/actracker-ui/pull/17).
- Tag sets were introduced in [API](https://github.com/marcinciapa/actracker-api/pull/45) and [UI](https://github.com/marcinciapa/actracker-ui/pull/18). 
...
- Charts were added to Dashboards in [API](https://github.com/marcinciapa/actracker-api/pull/48) and [UI](https://github.com/marcinciapa/actracker-ui/pull/20)